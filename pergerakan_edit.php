...
$layout->blocks["top"][] = "details";$page_layouts["pergerakan_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pergerakan_inline_edit.htm" : "pergerakan_edit.htm";
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pergerakan_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pergerakan_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["daripada_bahagian"] = $evalues["daripada_bahagian"];
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pergerakan";
		$options['firstTime'] = 1;
		
		$strTableName = $dpParams['strTableNames'][$d];
		
		if(!CheckSecurity(@$_SESSION["_".$strTableName."_OwnerID"],"Search")){
			$strTableName = "pergerakan";
			continue;
		}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pergerakan_view.php?".$viewlink."'\"");
