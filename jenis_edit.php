...
$layout->blocks["top"][] = "details";$page_layouts["jenis_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "jenis_inline_edit.htm" : "jenis_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["jid"] = urldecode(postvalue("editid1"));
$savedKeys["jid"] = urldecode(postvalue("editid1"));
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["jid"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: jenis_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: jenis_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("jid", $data)));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='jenis_view.php?".$viewlink."'\"");
