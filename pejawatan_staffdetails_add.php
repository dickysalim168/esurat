...
$layout->blocks["top"][] = "details";$page_layouts["pejawatan_staffdetails_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pejawatan_staffdetails_inline_add.htm";
else
	$templatefile = "pejawatan_staffdetails_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pejawatan_staffdetails_add.php');
...
//	processing dept_id - end
	...
	if(postvalue("table")=="surat_individu" && postvalue("field")=="iid") 
	{
		if(!array_key_exists("dept_id",$avalues))
			$avalues["dept_id"] = postvalue("category");
	}
	if(postvalue("table")=="UserView" && postvalue("field")=="iid") 
	{
		if(!array_key_exists("dept_id",$avalues))
			$avalues["dept_id"] = postvalue("category");
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pejawatan_staffdetails_edit.php?'.$keylink.'\'>'."Kemaskini".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pejawatan_staffdetails_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pejawatan_staffdetails_".$pageObject->getPageType().".php");
...
if(postvalue("table")=="surat_individu" && postvalue("field")=="iid") 
	$defvalues["dept_id"] = postvalue("category");
if(postvalue("table")=="UserView" && postvalue("field")=="iid") 
	$defvalues["dept_id"] = postvalue("category");
if($readavalues)
{
	...
	$defvalues["post_id"]=@$avalues["post_id"];
	$defvalues["dept_id"]=@$avalues["dept_id"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("post_id_label","<label for=\"".GetInputElementId("post_id", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("dept_id_label","<label for=\"".GetInputElementId("dept_id", $id, PAGE_ADD)."\">","</label>");
