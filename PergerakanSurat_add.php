...
$layout->blocks["top"][] = "details";$page_layouts["PergerakanSurat_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "PergerakanSurat_inline_add.htm";
else
	$templatefile = "PergerakanSurat_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: PergerakanSurat_add.php');
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="suratview")
	{
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'PergerakanSurat_edit.php?'.$keylink.'\'>'."Sunting".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'PergerakanSurat_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: PergerakanSurat_".$pageObject->getPageType().".php");
