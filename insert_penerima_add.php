...
if($inlineadd == ADD_INLINE)
	$templatefile = "insert_penerima_inline_add.htm";
else
	$templatefile = "insert_penerima_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: insert_penerima_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'insert_penerima_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'insert_penerima_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: insert_penerima_".$pageObject->getPageType().".php");
