...
$layout->blocks["top"][] = "details";$page_layouts["bahagian_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "bahagian_inline_add.htm";
else
	$templatefile = "bahagian_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: bahagian_add.php');
...
//	processing nama - end
//	processing penerangan - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_penerangan = $pageObject->getControl("penerangan", $id);
		$control_penerangan->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing penerangan - end
...
//	processing penerangan - start
			...
			if($inlineAddOption)
			{
				$control_penerangan->afterSuccessfulSave();
			}
//	processing penerangan - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'bahagian_edit.php?'.$keylink.'\'>'."Kemaskini".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'bahagian_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: bahagian_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["penerangan"]=@$avalues["penerangan"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("penerangan"))
		$xt->assign("penerangan_fieldblock",true);
	else
		$xt->assign("penerangan_tabfieldblock",true);
	$xt->assign("penerangan_label",true);
	if(isEnableSection508())
		$xt->assign_section("penerangan_label","<label for=\"".GetInputElementId("penerangan", $id, PAGE_ADD)."\">","</label>");
...
//	penerangan
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("penerangan", $data, $keylink);
		$showValues["penerangan"] = $value;
		$showFields[] = "penerangan";
				$showRawValues["penerangan"] = substr($data["penerangan"],0,100);
	}	
