<?php
include_once(getabspath("include/UserView_settings.php"));

function DisplayMasterTableInfo_UserView($params)
{
...
//$strSQL = "SELECT  surat.sid,  surat.tarikhTerima,  surat.Jenis,  surat.awam,  surat.agensi,  surat.bahagian,  surat.individu,  surat.jawatan,  surat.kategori,  surat.rujukanSurat,  surat.RingkasanKandungan,  surat.tarikhSurat,  surat.status,  surat_individu.iid,  pengguna.pid  FROM surat_individu  , surat  , pengguna  WHERE (surat.sid = surat_individu.sid) AND (surat_individu.iid = pengguna.iid)  ";
...
$layout->blocks["bare"][] = "mastergrid";$page_layouts["UserView_masterprint"] = $layout;


$showKeys = "";
if($detailtable=="pergerakan")
{
...
//	status - 
	...
	$xt->display("UserView_masterprint.htm");
	$strTableName=$oldTableName;
