...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="surat" && $field=="sid")
	{
		$value = "<a target='_self' href='surat_edit.php?editid1=".$data['sid']."'>Kemaskini</a>";;
		return $value;
	}
				if($table=="surat" && $field=="Jenis")
	{
...
$value = "Awam";  
		...
		return $value;
				...
				if($table=="surat" && $field=="tindakan")
	{
		
//count jumlah penerima
$sql2 = "SELECT count(1) FROM surat_individu a, pengguna b WHERE a.Penerima = b.pid AND b.Status = 'Aktif' AND a.sid = '$data[sid]'";
$rs2 = CustomQuery($sql2);
$data2 = db_fetch_array($rs2);

//count jumlah penerima yang dah selesai
$sql3 = "SELECT count(1) FROM surat_individu a, pengguna b WHERE a.Penerima = b.pid AND b.Status = 'Aktif' AND a.sid = '$data[sid]' AND a.Status = '1'";
$rs3 = CustomQuery($sql3);
$data3 = db_fetch_array($rs3);

$sisa = $data2["count(1)"] - $data3["count(1)"];

$value = $data3["count(1)"]."/".$data2["count(1)"]."(".$sisa.")";;
		return $value;
	}
				if($table=="surat" && $field=="pilihan")
	{
		$value = "<a target='_self' href='surat_edit.php?editid1=".$data['sid']."'>Lihat</a>";;
		return $value;
	}
				if($table=="surat" && $field=="TindakanTotal")
	{
...
if($sisa > 0)
{
			$sqlu = "UPDATE surat set TindakanTotal = 'Belum Selesai' WHERE sid = '$data[sid]'";
			CustomQuery($sqlu);	
}
else
{
			$sqlu = "UPDATE surat set TindakanTotal = 'Selesai' WHERE sid = '$data[sid]'";
			CustomQuery($sqlu);
}

if($data["TindakanTotal"] == '' || $data["TindakanTotal"] == 'Belum Selesai')
{
$value = "<font color='red'>Belum Selesai</font>";

}
else
{
$value = "<font color='green'>".$data["TindakanTotal"]."</font>";
}

};
		return $value;
	}
				if($table=="surat_individu" && $field=="siid")
	{
		
$value = "<a onClick='return confirm(\"Adakah Anda Pasti?\")' href='scansurat/deletepenerima.php?siid=$data[siid]'>Hapus Dokumen</a>";;
		return $value;
	}
				if($table=="pengguna" && $field=="Status")
	{
		if($data['Status'] == 'Aktif')
{
$value = "<font color='green'>Aktif</font>";
}
else
{
$value = "<font color='red'>Tidak Aktif</font>";
};
		return $value;
	}
				if($table=="Viewer" && $field=="Penghantar")
	{
		$value = $_SESSION['StaffInfo'][$value];;
		return $value;
	}
				if($table=="Viewer" && $field=="Penerima")
	{
		$value = $_SESSION['StaffInfo'][$value];;
		return $value;
	}
				if($table=="surat_view" && $field=="Penghantar")
	{
		$value = $_SESSION['StaffInfo'][$data["Penghantar"]];;
		return $value;
	}
				if($table=="surat_view" && $field=="Penerima")
	{
		$value = $_SESSION['StaffInfo'][$value];;
		return $value;
	}
				if($table=="surat_view" && $field=="sid")
	{
		if($_SESSION["Level"] == 1)
{
$value = "
<a href='surat_editor_edit.php?editid1=$data[siid]'>Lihat Surat</a>
";
}
else
{
$value = "
<a href='Viewer_edit.php?editid1=$data[siid]'>Lihat Surat</a>
";
};
		return $value;
	}
				if($table=="menuview" && $field=="sid")
	{
		$value = "<a target='_self' href='surat_edit.php?editid1=".$data['sid']."'>Kemaskini</a>";;
		return $value;
	}
				if($table=="menuview" && $field=="Jenis")
	{
		$sql = "SELECT * from awam";
$rs = CustomQuery($sql);
while($value = db_fetch_array($rs))
...
$value = "Awam -<br> ".$awam[$data["awam"]];  
		...
		return $value;
				...
				if($table=="surat_editor" && $field=="Penghantar")
	{
		$value = $_SESSION['StaffInfo'][$value];;
		return $value;
	}
				if($table=="surat_editor" && $field=="Penerima")
	{
		$value = $_SESSION['StaffInfo'][$value];;
		return $value;
	}
				if($table=="surat_maklumatsurat" && $field=="sid")
	{
		$value = "<a target='_self' href='surat_edit.php?editid1=".$data['sid']."'>Kemaskini</a>";;
		return $value;
	}
				if($table=="surat_maklumatsurat" && $field=="Jenis")
	{
		$sql = "SELECT * from awam";
$rs = CustomQuery($sql);
while($value = db_fetch_array($rs))
...
$value = "Awam -<br> ".$awam[$data["awam"]];  
		...
		return $value;
				...
				if($table=="surat_PTG" && $field=="sid")
	{
		$value = "<a target='_self' href='surat_edit.php?editid1=".$data['sid']."'>Kemaskini</a>";;
		return $value;
	}
				if($table=="surat_PTG" && $field=="Jenis")
	{
		$sql = "SELECT * from awam";
$rs = CustomQuery($sql);
while($value = db_fetch_array($rs))
...
$value = "Awam -<br> ".$awam[$data["awam"]];  
		...
		return $value;
				...
				if($table=="surat_AgensiAwam" && $field=="sid")
	{
		$value = "<a target='_self' href='surat_edit.php?editid1=".$data['sid']."'>Kemaskini</a>";;
		return $value;
	}
				if($table=="surat_AgensiAwam" && $field=="Jenis")
	{
		$sql = "SELECT * from awam";
$rs = CustomQuery($sql);
while($value = db_fetch_array($rs))
...
$value = "Awam -<br> ".$awam[$data["awam"]];  
		...
		return $value;
				...
				if($table=="surat_Awam" && $field=="sid")
	{
		$value = "<a target='_self' href='surat_edit.php?editid1=".$data['sid']."'>Kemaskini</a>";;
		return $value;
	}
				if($table=="surat_Awam" && $field=="Jenis")
	{
		$sql = "SELECT * from awam";
$rs = CustomQuery($sql);
while($value = db_fetch_array($rs))
...
$value = "Awam -<br> ".$awam[$data["awam"]];  
		...
		return $value;
				...
				if($table=="suratview2" && $field=="sid")
	{
		$value = "<a target='_self' href='surat_edit.php?editid1=".$data['sid']."'>Kemaskini</a>";;
		return $value;
	}
				if($table=="suratview2" && $field=="Jenis")
	{
		$sql = "SELECT * from awam";
$rs = CustomQuery($sql);
while($value = db_fetch_array($rs))
...
$value = "Awam -<br> ".$awam[$data["awam"]];  
		...
		return $value;
				...
				if($table=="TambahPenerima" && $field=="Status")
	{
		if($data['Status'] == 'Aktif')
{
$value = "<font color='green'>Aktif</font>";
...
function GetLWWhere($field, $ptype, $table = "")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="surat" && $field=="individu")
	{
		return " pid > 1 ";
	}
				if($table=="menuview" && $field=="individu")
	{
		return " pid > 1 ";
	}
				if($table=="surat_PTG" && $field=="individu" && $ptype=="edit")
	{
		return " pid > 1 ";
	}
				if($table=="surat_AgensiAwam" && $field=="agensi")
	{
		return " jenis = " . $_SESSION["Jenis"];
	}
	return "";
}
...
function GetDefaultValue($field, $ptype, $table="")
{
				...
				if($table=="surat" && $field=="Jenis")
	{
		return 1;
				...
				if($table=="surat" && $field=="kategori")
	{
		return "Surat";
	}
				if($table=="surat" && $field=="tarikhKemasukkan")
	{
		return now();
	}
				if($table=="menuview" && $field=="Jenis")
				...
				if($table=="surat_PTG" && $field=="Jenis")
	{
		return 1;
				...
				if($table=="surat_PTG" && $field=="kategori")
	{
		return "Biasa";
	}
				if($table=="surat_AgensiAwam" && $field=="Jenis")
	{
		return 1;
				...
				if($table=="surat_AgensiAwam" && $field=="kategori")
	{
		return "Biasa";
	}
				if($table=="surat_Awam" && $field=="Jenis")
	{
		return 1;
				...
				if($table=="surat_Awam" && $field=="kategori")
	{
		return "Biasa";
	}
				if($table=="suratview2" && $field=="Jenis")
