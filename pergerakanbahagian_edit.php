...
$layout->blocks["top"][] = "details";$page_layouts["pergerakanbahagian_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pergerakanbahagian_inline_edit.htm" : "pergerakanbahagian_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["pbid"] = urldecode(postvalue("editid1"));
$savedKeys["pbid"] = urldecode(postvalue("editid1"));
...
//	processing KepadaBahagian - begin
	$condition = 1;

	if($condition)
	{
		$control_KepadaBahagian = $pageObject->getControl("KepadaBahagian", $id);
		$control_KepadaBahagian->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["pbid"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: pergerakanbahagian_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pergerakanbahagian_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["KepadaBahagian"] = $evalues["KepadaBahagian"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("KepadaBahagian_label","<label for=\"".GetInputElementId("KepadaBahagian", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("pbid", $data)));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pergerakanbahagian_view.php?".$viewlink."'\"");
