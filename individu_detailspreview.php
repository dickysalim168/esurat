...
$layout = new TLayout("detailspreview","PurificAqua","MobileAqua");
...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["individu_detailspreview"] = $layout;
...
if($mastertable == "bahagian_agensi")
{
	$where = "";
		$where .= GetFullFieldName("baid", $strTableName, false)."=".make_db_value("baid",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) {
	...
	//	jaid - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("jaid", $data, $keylink);
			$row["jaid_value"] = $value;
	//	namaBahagian - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("namaBahagian", $data, $keylink);
			$row["namaBahagian_value"] = $value;
	//	jawatan - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("jawatan", $data, $keylink);
			$row["jawatan_value"] = $value;
	//	jid - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("jid", $data, $keylink);
			$row["jid_value"] = $value;
		$rowinfo[] = $row;
		$data = $cipherer->DecryptFetchedArray($rs);
	}
...
$xt->load_template("individu_detailspreview.htm");
