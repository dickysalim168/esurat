...
$layout->blocks["top"][] = "pdf";$page_layouts["document_print"] = $layout;
...
//	process selection
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["did"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
...
$arr = array();
$arr['fName'] = "file";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("file");
$fieldsArr[] = $arr;
...
//	file - Database Image
			$record["file_value"] = $pageObject->showDBValue("file", $data, $keylink);
			$record["file_class"] = $pageObject->fieldAlignClass("file");
			if($col<$colsonpage)
...
//	include proper masterprint.php code
	...
	$params = array("detailtable"=>"document","keys"=>$masterkeys);
	$master = array();
...
$xt->assign("file_fieldheadercolumn",true);
$xt->assign("file_fieldheader",true);
$xt->assign("file_fieldcolumn",true);
$xt->assign("file_fieldfootercolumn",true);
