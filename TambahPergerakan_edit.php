...
$templatefile = ($inlineedit == EDIT_INLINE) ? "TambahPergerakan_inline_edit.htm" : "TambahPergerakan_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["staff_id"] = urldecode(postvalue("editid1"));
$savedKeys["staff_id"] = urldecode(postvalue("editid1"));
...
//	processing lantikan_date - begin
	$condition = 1;

	if($condition)
	{
		$control_lantikan_date = $pageObject->getControl("lantikan_date", $id);
		$control_lantikan_date->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing lantikan_date - end
//	processing sah_date - begin
	$condition = 1;

	if($condition)
	{
		$control_sah_date = $pageObject->getControl("sah_date", $id);
		$control_sah_date->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing sah_date - end
//	processing pencen_date - begin
	$condition = 1;

	if($condition)
	{
		$control_pencen_date = $pageObject->getControl("pencen_date", $id);
		$control_pencen_date->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing pencen_date - end
//	processing gaji_date - begin
	$condition = 1;

	if($condition)
	{
		$control_gaji_date = $pageObject->getControl("gaji_date", $id);
		$control_gaji_date->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing gaji_date - end
//	processing skim_pencen - begin
	$condition = 1;

	if($condition)
	{
		$control_skim_pencen = $pageObject->getControl("skim_pencen", $id);
		$control_skim_pencen->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing skim_pencen - end
//	processing tamatkontrak_date - begin
	$condition = 1;

	if($condition)
	{
		$control_tamatkontrak_date = $pageObject->getControl("tamatkontrak_date", $id);
		$control_tamatkontrak_date->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing tamatkontrak_date - end
//	processing nofail - begin
	$condition = 1;

	if($condition)
	{
		$control_nofail = $pageObject->getControl("nofail", $id);
		$control_nofail->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing nofail - end
//	processing grade_id - begin
	$condition = 1;

	if($condition)
	{
		$control_grade_id = $pageObject->getControl("grade_id", $id);
		$control_grade_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing grade_id - end
//	processing start_date - begin
	$condition = 1;

	if($condition)
	{
		$control_start_date = $pageObject->getControl("start_date", $id);
		$control_start_date->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing start_date - end
//	processing scheme_id - begin
	$condition = 1;

	if($condition)
	{
		$control_scheme_id = $pageObject->getControl("scheme_id", $id);
		$control_scheme_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing type - end
//	processing level - begin
	$condition = 1;

	if($condition)
	{
		$control_level = $pageObject->getControl("level", $id);
		$control_level->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["staff_id"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: TambahPergerakan_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: TambahPergerakan_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["lantikan_date"] = $evalues["lantikan_date"];
	$data["sah_date"] = $evalues["sah_date"];
	$data["pencen_date"] = $evalues["pencen_date"];
	$data["gaji_date"] = $evalues["gaji_date"];
	$data["skim_pencen"] = $evalues["skim_pencen"];
	$data["tamatkontrak_date"] = $evalues["tamatkontrak_date"];
	$data["nofail"] = $evalues["nofail"];
	$data["grade_id"] = $evalues["grade_id"];
	$data["start_date"] = $evalues["start_date"];
	$data["scheme_id"] = $evalues["scheme_id"];
	$data["username"] = $evalues["username"];
	$data["password"] = $evalues["password"];
	$data["type"] = $evalues["type"];
	$data["level"] = $evalues["level"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("lantikan_date_label","<label for=\"".GetInputElementId("lantikan_date", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("sah_date_label","<label for=\"".GetInputElementId("sah_date", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("pencen_date_label","<label for=\"".GetInputElementId("pencen_date", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("gaji_date_label","<label for=\"".GetInputElementId("gaji_date", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("skim_pencen_label","<label for=\"".GetInputElementId("skim_pencen", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("tamatkontrak_date_label","<label for=\"".GetInputElementId("tamatkontrak_date", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("nofail_label","<label for=\"".GetInputElementId("nofail", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("grade_id_label","<label for=\"".GetInputElementId("grade_id", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("start_date_label","<label for=\"".GetInputElementId("start_date", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("scheme_id_label","<label for=\"".GetInputElementId("scheme_id", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("level_label","<label for=\"".GetInputElementId("level", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("staff_id", $data)));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='TambahPergerakan_view.php?".$viewlink."'\"");
