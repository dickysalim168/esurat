...
$layout->blocks["top"][] = "details";$page_layouts["pejawatan_staffdetails_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pejawatan_staffdetails_inline_edit.htm" : "pejawatan_staffdetails_edit.htm";
...
//	processing icno - end
//	processing post_id - begin
	$condition = 1;

	if($condition)
	{
		$control_post_id = $pageObject->getControl("post_id", $id);
		$control_post_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing post_id - end
//	processing dept_id - begin
	$condition = 1;

	if($condition)
	{
		$control_dept_id = $pageObject->getControl("dept_id", $id);
		$control_dept_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pejawatan_staffdetails_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pejawatan_staffdetails_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["post_id"] = $evalues["post_id"];
	$data["dept_id"] = $evalues["dept_id"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("post_id_label","<label for=\"".GetInputElementId("post_id", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("dept_id_label","<label for=\"".GetInputElementId("dept_id", $id, PAGE_EDIT)."\">","</label>");
...
//	staff_id - 
	$value = $pageObject->showDBValue("staff_id", $data, $keylink);
	$showValues["staff_id"] = $value;
	$showFields[] = "staff_id";
		$showRawValues["staff_id"] = substr($data["staff_id"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pejawatan_staffdetails";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pejawatan_staffdetails_view.php?".$viewlink."'\"");
