...
$layout->blocks["top"][] = "details";$page_layouts["Copy_of_pejawatan_staffdetails_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "Copy_of_pejawatan_staffdetails_inline_add.htm";
else
	$templatefile = "Copy_of_pejawatan_staffdetails_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: Copy_of_pejawatan_staffdetails_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'Copy_of_pejawatan_staffdetails_edit.php?'.$keylink.'\'>'."Sunting".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'Copy_of_pejawatan_staffdetails_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: Copy_of_pejawatan_staffdetails_".$pageObject->getPageType().".php");
