...
$layout = new TLayout("add2","Purific1Aqua","MobileAqua");
...
$layout->blocks["top"][] = "details";$page_layouts["pergerakan_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pergerakan_inline_add.htm";
else
	$templatefile = "pergerakan_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pergerakan_add.php');
...
//	processing daripada_bahagian - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_daripada_bahagian = $pageObject->getControl("daripada_bahagian", $id);
		$control_daripada_bahagian->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="surat")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["sid"]==""){
			$avalues["sid"] = prepare_for_db("sid",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pergerakan_edit.php?'.$keylink.'\'>'."Sunting".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pergerakan_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pergerakan_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["pid"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["pid"]=postvalue("editid1");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="surat")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["sid"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
	
}

if($readavalues)
{
	$defvalues["tarikh"]=@$avalues["tarikh"];
	$defvalues["daripada_bahagian"]=@$avalues["daripada_bahagian"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("tarikh_label","<label for=\"".GetInputElementId("tarikh", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("daripada_bahagian_label","<label for=\"".GetInputElementId("daripada_bahagian", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("catatan_label","<label for=\"".GetInputElementId("catatan", $id, PAGE_ADD)."\">","</label>");
