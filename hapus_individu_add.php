...
$layout->blocks["top"][] = "details";$page_layouts["hapus_individu_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "hapus_individu_inline_add.htm";
else
	$templatefile = "hapus_individu_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: hapus_individu_add.php');
...
//	processing sid - start
	...
	if($inlineAddOption)
	{
		...
		$control_sid->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing sid - end
//	processing iid - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_iid = $pageObject->getControl("iid", $id);
		$control_iid->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing iid - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'hapus_individu_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'hapus_individu_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: hapus_individu_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["siid"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["siid"]=postvalue("editid1");
	}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("sid_label","<label for=\"".GetInputElementId("sid", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("iid_label","<label for=\"".GetInputElementId("iid", $id, PAGE_ADD)."\">","</label>");
...
//	iid
	...
	{	
		...
		$showValues["iid"] = $value;
		$showFields[] = "iid";
				$showRawValues["iid"] = substr($data["iid"],0,100);
	}	
//	nama
	$display = false;
