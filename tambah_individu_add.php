...
$layout = new TLayout("add3","FusionBlueWave","MobileBlueWave");
...
$layout->blocks["top"][] = "details";$page_layouts["tambah_individu_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "tambah_individu_inline_add.htm";
else
	$templatefile = "tambah_individu_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: tambah_individu_add.php');
...
//	processing lid - start
	...
	if($inlineAddOption)
	{
		$control_lid = $pageObject->getControl("lid", $id);
		$control_lid->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing lid - end
//	processing pass - start
	...
	if($inlineAddOption)
	{
		...
		$control_pass->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing pass - end
...
//	processing baid - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_baid = $pageObject->getControl("baid", $id);
		$control_baid->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing baid - end
//	processing jaid - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_jaid = $pageObject->getControl("jaid", $id);
		$control_jaid->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing jaid - end
...
//	processing lid - start
			...
			if($inlineAddOption)
			{
				$control_lid->afterSuccessfulSave();
			}
//	processing lid - end
//	processing pass - start
			$inlineAddOption = true;
...
//	processing baid - start
			...
			if($inlineAddOption)
			{
				$control_baid->afterSuccessfulSave();
			}
//	processing baid - end
//	processing jaid - start
			$inlineAddOption = true;
			if($inlineAddOption)
			{
				$control_jaid->afterSuccessfulSave();
			}
//	processing jaid - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'tambah_individu_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'tambah_individu_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: tambah_individu_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["iid"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["iid"]=postvalue("editid1");
	}
...
if($readavalues)
{
	...
	$defvalues["baid"]=@$avalues["baid"];
	$defvalues["jaid"]=@$avalues["jaid"];
	$defvalues["pass"]=@$avalues["pass"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("baid_label","<label for=\"".GetInputElementId("baid", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("jaid_label","<label for=\"".GetInputElementId("jaid", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("pass_label","<label for=\"".GetInputElementId("pass", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("lid"))
		$xt->assign("lid_fieldblock",true);
	else
		$xt->assign("lid_tabfieldblock",true);
	$xt->assign("lid_label",true);
	if(isEnableSection508())
		$xt->assign_section("lid_label","<label for=\"".GetInputElementId("lid", $id, PAGE_ADD)."\">","</label>");
...
//	baid
	...
	{	
		...
		$showValues["baid"] = $value;
		$showFields[] = "baid";
				$showRawValues["baid"] = substr($data["baid"],0,100);
	}	
//	jaid
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		...
		$showValues["jaid"] = $value;
		$showFields[] = "jaid";
				$showRawValues["jaid"] = substr($data["jaid"],0,100);
	}	
//	pass
	$display = false;
	if($inlineadd==ADD_MASTER)
...
//	lid
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("lid", $data, $keylink);
		$showValues["lid"] = $value;
		$showFields[] = "lid";
				$showRawValues["lid"] = substr($data["lid"],0,100);
	}	
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "tambah individu";
		$options['firstTime'] = 1;
