...
include("include/PergerakanSurat_variables.php");
...
$layout = new TLayout("edit2","Purific1Aqua","MobileAqua");
...
$layout->blocks["top"][] = "details";$page_layouts["PergerakanSurat_edit"] = $layout;
...
$layout->blocks["top"][] = "search";$page_layouts["pergerakanbahagian_list"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "PergerakanSurat_inline_edit.htm" : "PergerakanSurat_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["pid"] = urldecode(postvalue("editid1"));
$savedKeys["pid"] = urldecode(postvalue("editid1"));
...
if($inlineedit!=EDIT_INLINE)
{
	...
	{
		$ids = $id;
			$dpPermis = $pageObject->getPermissions("pergerakanbahagian");
		if($dpPermis['search'] || $dpPermis['edit']){
			$mKeys["pergerakanbahagian"] = $pageObject->pSet->getMasterKeysByDetailTable("pergerakanbahagian");
			$dpParams['strTableNames'][] = "pergerakanbahagian";
			$dpParams['ids'][] = ++$ids;
		}
...
//	processing daripada_bahagian - begin
	$condition = 1;

	if($condition)
	{
		$control_daripada_bahagian = $pageObject->getControl("daripada_bahagian", $id);
		$control_daripada_bahagian->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing daripada_bahagian - end
...
//	processing catatan - end
	...
	//	processing tarikh - end
			//	processing daripada_bahagian - begin
				...
				if($condition)
				{
					$control_daripada_bahagian->afterSuccessfulSave();
				}
	//	processing daripada_bahagian - end
			//	processing catatan - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["pid"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: PergerakanSurat_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: PergerakanSurat_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
//	show readonly fields
	$pageObject->readOnlyFields["daripada_bahagian"] = $pageObject->showDBValue("daripada_bahagian", $data);
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("daripada_bahagian"))
		$xt->assign("daripada_bahagian_fieldblock",true);
	else
		$xt->assign("daripada_bahagian_tabfieldblock",true);
	$xt->assign("daripada_bahagian_label",true);
	if(isEnableSection508())
		$xt->assign_section("daripada_bahagian_label","<label for=\"".GetInputElementId("daripada_bahagian", $id, PAGE_EDIT)."\">","</label>");
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("pid", $data)));
...
//	detail tables
	$showDetailKeys["pergerakanbahagian"]["masterkey1"] = $data["pid"];		
...
//	daripada_bahagian - Custom
	$value = $pageObject->showDBValue("daripada_bahagian", $data, $keylink);
	$showValues["daripada_bahagian"] = $value;
	$showFields[] = "daripada_bahagian";
		$showRawValues["daripada_bahagian"] = substr($data["daripada_bahagian"],0,100);

//	kepada_bahagian - Custom
	$value = $pageObject->showDBValue("kepada_bahagian", $data, $keylink);
	$showValues["kepada_bahagian"] = $value;
	$showFields[] = "kepada_bahagian";
		$showRawValues["kepada_bahagian"] = substr($data["kepada_bahagian"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "PergerakanSurat";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='PergerakanSurat_view.php?".$viewlink."'\"");
