...
$layout->blocks["top"][] = "details";$page_layouts["bahagian_agensi_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "bahagian_agensi_inline_edit.htm" : "bahagian_agensi_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["baid"] = urldecode(postvalue("editid1"));
$savedKeys["baid"] = urldecode(postvalue("editid1"));
...
//	processing jid - begin
	$condition = 1;

	if($condition)
	{
		$control_jid = $pageObject->getControl("jid", $id);
		$control_jid->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing jid - end
//	processing namaBahagian - begin
	$condition = 1;

	if($condition)
	{
		$control_namaBahagian = $pageObject->getControl("namaBahagian", $id);
		$control_namaBahagian->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["baid"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: bahagian_agensi_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: bahagian_agensi_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["jid"] = $evalues["jid"];
	$data["namaBahagian"] = $evalues["namaBahagian"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("jid_label","<label for=\"".GetInputElementId("jid", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("namaBahagian_label","<label for=\"".GetInputElementId("namaBahagian", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("baid", $data)));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='bahagian_agensi_view.php?".$viewlink."'\"");
