...
$layout->blocks["top"][] = "details";$page_layouts["surat_editor2_edit"] = $layout;
...
$layout->blocks["bare"][] = "1";$page_layouts["surat_editor2_edit_PTG1"] = $layout;
...
$layout->skins["1"] = "fields";
$layout->blocks["bare"][] = "1";$page_layouts["surat_editor2_edit_Agensi1"] = $layout;
...
$layout->skins["1"] = "fields";
$layout->blocks["bare"][] = "1";$page_layouts["surat_editor2_edit_Awam1"] = $layout;
...
$templatefile = "surat_editor2_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["rujukanSurat"] = urldecode(postvalue("editid1"));
$savedKeys["rujukanSurat"] = urldecode(postvalue("editid1"));
...
//	processing siid - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_siid = $pageObject->getControl("siid", $id);
		$control_siid->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing siid - end
//	processing Tarikh - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_Tarikh = $pageObject->getControl("Tarikh", $id);
		$control_Tarikh->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Tarikh - end
//	processing sid - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_sid = $pageObject->getControl("sid", $id);
		$control_sid->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing sid - end
//	processing Penghantar - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_Penghantar = $pageObject->getControl("Penghantar", $id);
		$control_Penghantar->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Penghantar - end
//	processing Penerima - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_Penerima = $pageObject->getControl("Penerima", $id);
		$control_Penerima->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Penerima - end
//	processing Catatan - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_Catatan = $pageObject->getControl("Catatan", $id);
		$control_Catatan->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing Email_Status - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_Email_Status = $pageObject->getControl("Email_Status", $id);
		$control_Email_Status->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Email_Status - end
//	processing rujukanSurat - begin
	...
	if($condition)
	{
		...
		//	update key value
		if($control_rujukanSurat->getWebValue()!==false)
			$keys["rujukanSurat"] = $control_rujukanSurat->getWebValue();
	}
...
//	processing RingkasanKandungan - end
	...
	//	processing Tarikh - end
			//	processing sid - begin
				...
				if($condition)
				{
					$control_sid->afterSuccessfulSave();
				}
	//	processing sid - end
			//	processing Penghantar - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
	...
	//	processing Status - end
			//	processing Email_Status - begin
				...
				if($condition)
				{
					$control_Email_Status->afterSuccessfulSave();
				}
	//	processing Email_Status - end
			//	processing rujukanSurat - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["rujukanSurat"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: surat_editor2_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: surat_editor2_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["siid"] = $evalues["siid"];
	$data["Tarikh"] = $evalues["Tarikh"];
	$data["sid"] = $evalues["sid"];
	$data["Penghantar"] = $evalues["Penghantar"];
	$data["Penerima"] = $evalues["Penerima"];
	$data["Catatan"] = $evalues["Catatan"];
	$data["Status"] = $evalues["Status"];
	$data["Email_Status"] = $evalues["Email_Status"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("siid"))
		$xt->assign("siid_fieldblock",true);
	else
		$xt->assign("siid_tabfieldblock",true);
	$xt->assign("siid_label",true);
	if(isEnableSection508())
		$xt->assign_section("siid_label","<label for=\"".GetInputElementId("siid", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("Tarikh"))
		$xt->assign("Tarikh_fieldblock",true);
	else
		$xt->assign("Tarikh_tabfieldblock",true);
	$xt->assign("Tarikh_label",true);
	if(isEnableSection508())
		$xt->assign_section("Tarikh_label","<label for=\"".GetInputElementId("Tarikh", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("sid"))
		$xt->assign("sid_fieldblock",true);
	else
		$xt->assign("sid_tabfieldblock",true);
	$xt->assign("sid_label",true);
	if(isEnableSection508())
		$xt->assign_section("sid_label","<label for=\"".GetInputElementId("sid", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("Penghantar"))
		$xt->assign("Penghantar_fieldblock",true);
	else
		$xt->assign("Penghantar_tabfieldblock",true);
	$xt->assign("Penghantar_label",true);
	if(isEnableSection508())
		$xt->assign_section("Penghantar_label","<label for=\"".GetInputElementId("Penghantar", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("Penerima"))
		$xt->assign("Penerima_fieldblock",true);
	else
		$xt->assign("Penerima_tabfieldblock",true);
	$xt->assign("Penerima_label",true);
	if(isEnableSection508())
		$xt->assign_section("Penerima_label","<label for=\"".GetInputElementId("Penerima", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("Catatan"))
		$xt->assign("Catatan_fieldblock",true);
	else
		$xt->assign("Catatan_tabfieldblock",true);
	$xt->assign("Catatan_label",true);
	if(isEnableSection508())
		$xt->assign_section("Catatan_label","<label for=\"".GetInputElementId("Catatan", $id, PAGE_EDIT)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("Email_Status"))
		$xt->assign("Email_Status_fieldblock",true);
	else
		$xt->assign("Email_Status_tabfieldblock",true);
	$xt->assign("Email_Status_label",true);
	if(isEnableSection508())
		$xt->assign_section("Email_Status_label","<label for=\"".GetInputElementId("Email_Status", $id, PAGE_EDIT)."\">","</label>");
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("rujukanSurat", $data)));
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "surat_editor2";
		$options['firstTime'] = 1;
		
		$strTableName = $dpParams['strTableNames'][$d];
		
		if(!CheckSecurity(@$_SESSION["_".$strTableName."_OwnerID"],"Search")){
			$strTableName = "surat_editor2";
			continue;
		}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='surat_editor2_view.php?".$viewlink."'\"");
