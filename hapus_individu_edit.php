...
$layout = new TLayout("edit3","FusionBlueWave","MobileBlueWave");
...
$layout->blocks["top"][] = "details";$page_layouts["hapus_individu_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "hapus_individu_inline_edit.htm" : "hapus_individu_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["siid"] = urldecode(postvalue("editid1"));
$savedKeys["siid"] = urldecode(postvalue("editid1"));
...
//	processing nama - end
//	processing baid - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_baid = $pageObject->getControl("baid", $id);
		$control_baid->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing baid - end
//	processing jaid - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_jaid = $pageObject->getControl("jaid", $id);
		$control_jaid->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["siid"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: hapus_individu_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: hapus_individu_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["baid"] = $evalues["baid"];
	$data["jaid"] = $evalues["jaid"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("siid", $data)));
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "hapus individu";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='hapus_individu_view.php?".$viewlink."'\"");
