...
$layout->blocks["top"][] = "search";$page_layouts["PergerakanSurat_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "PergerakanSurat_search.htm";
...
$params['shortTableName'] = 'PergerakanSurat';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("daripada_bahagian"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("daripada_bahagian")] = GetTableURL($pageObject->pSet->getLookupTable("daripada_bahagian"));
	
	$pageObject->fillFieldToolTips("daripada_bahagian");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("daripada_bahagian");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "daripada_bahagian";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("daripada_bahagian_label","<label for=\"".GetInputElementId("daripada_bahagian", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("daripada_bahagian_label", true);
	
	$xt->assign("daripada_bahagian_fieldblock", true);
	$xt->assignbyref("daripada_bahagian_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("daripada_bahagian_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("daripada_bahagian_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_daripada_bahagian", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("daripada_bahagian");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"daripada_bahagian", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"daripada_bahagian", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
