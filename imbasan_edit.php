...
$layout->blocks["top"][] = "details";$page_layouts["imbasan_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "imbasan_inline_edit.htm" : "imbasan_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["imid"] = urldecode(postvalue("editid1"));
$savedKeys["imid"] = urldecode(postvalue("editid1"));
...
//	processing sid - end
//	processing Imbasan - begin
	$condition = 1;

	if($condition)
	{
		$control_Imbasan = $pageObject->getControl("Imbasan", $id);
		$control_Imbasan->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Imbasan - end
	...
	//	processing sid - end
			//	processing Imbasan - begin
				...
				if($condition)
				{
					$control_Imbasan->afterSuccessfulSave();
				}
	//	processing Imbasan - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["imid"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: imbasan_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: imbasan_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("Imbasan"))
		$xt->assign("Imbasan_fieldblock",true);
	else
		$xt->assign("Imbasan_tabfieldblock",true);
	$xt->assign("Imbasan_label",true);
	if(isEnableSection508())
		$xt->assign_section("Imbasan_label","<label for=\"".GetInputElementId("Imbasan", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("imid", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["imid"]));


//	imid - 
	$value = $pageObject->showDBValue("imid", $data, $keylink);
	$showValues["imid"] = $value;
	$showFields[] = "imid";
		$showRawValues["imid"] = substr($data["imid"],0,100);
...
//	Imbasan - 
	$value = $pageObject->showDBValue("Imbasan", $data, $keylink);
	$showValues["Imbasan"] = $value;
	$showFields[] = "Imbasan";
		$showRawValues["Imbasan"] = substr($data["Imbasan"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='imbasan_view.php?".$viewlink."'\"");
