...
$layout = new TLayout("add2","PurificAqua","MobileAqua");
...
$layout->blocks["top"][] = "details";$page_layouts["bahagian_agensi_add"] = $layout;
...
$layout->blocks["top"][] = "search";$page_layouts["individu_list"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "bahagian_agensi_inline_add.htm";
else
	$templatefile = "bahagian_agensi_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: bahagian_agensi_add.php');
...
if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER || $inlineadd==ADD_POPUP)
{
	...
	if($pageObject->isShowDetailTables  && !isMobile())
	{
		...
		$countDetailsIsShow = 0;
			$dpPermis = $pageObject->getPermissions("individu");
		if ($dpPermis['add']){
			$countDetailsIsShow ++;
			$mKeys["individu"] = $pageObject->pSet->getMasterKeysByDetailTable("individu");
			$dpParams['strTableNames'][] = "individu";
			$dpParams['ids'][] = ++$ids;
		}
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing jid - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_jid = $pageObject->getControl("jid", $id);
		$control_jid->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing jid - end
//	processing namaBahagian - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_namaBahagian = $pageObject->getControl("namaBahagian", $id);
		$control_namaBahagian->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing namaBahagian - end
...
//	processing jid - start
			...
			if($inlineAddOption)
			{
				$control_jid->afterSuccessfulSave();
			}
//	processing jid - end
//	processing namaBahagian - start
			$inlineAddOption = true;
			if($inlineAddOption)
			{
				$control_namaBahagian->afterSuccessfulSave();
			}
//	processing namaBahagian - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'bahagian_agensi_edit.php?'.$keylink.'\'>'."Kemaskini".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'bahagian_agensi_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: bahagian_agensi_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["baid"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["baid"]=postvalue("editid1");
	}
...
if(postvalue("table")=="surat" && postvalue("field")=="dariBahagian") 
	$defvalues["jid"] = postvalue("category");
if($readavalues)
{
	$defvalues["jid"]=@$avalues["jid"];
	$defvalues["namaBahagian"]=@$avalues["namaBahagian"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("jid"))
		$xt->assign("jid_fieldblock",true);
	else
		$xt->assign("jid_tabfieldblock",true);
	$xt->assign("jid_label",true);
	if(isEnableSection508())
		$xt->assign_section("jid_label","<label for=\"".GetInputElementId("jid", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("namaBahagian"))
		$xt->assign("namaBahagian_fieldblock",true);
	else
		$xt->assign("namaBahagian_tabfieldblock",true);
	$xt->assign("namaBahagian_label",true);
	if(isEnableSection508())
		$xt->assign_section("namaBahagian_label","<label for=\"".GetInputElementId("namaBahagian", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$showDetailKeys["individu"]["masterkey1"] = $data["baid"];	
...
//	jid
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("jid", $data, $keylink);
		$showValues["jid"] = $value;
		$showFields[] = "jid";
				$showRawValues["jid"] = substr($data["jid"],0,100);
	}	
//	namaBahagian
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("namaBahagian", $data, $keylink);
		$showValues["namaBahagian"] = $value;
		$showFields[] = "namaBahagian";
				$showRawValues["namaBahagian"] = substr($data["namaBahagian"],0,100);
	}	
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "bahagian_agensi";
		$options['firstTime'] = 1;
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="surat" && postvalue("field")=="dariBahagian") 
	{
		$lookupFieldName = "dariBahagian";
		$lookupTable = "bahagian_agensi";
		$linkFieldName = "baid";
		$dispfield = "namaBahagian";
		// Table which contain lookup field!
		$lookupPSet = new ProjectSettings("surat", PAGE_ADD);
		$nLookupType = $lookupPSet->getLookupType($lookupFieldName);
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "surat", "dariBahagian");
			...
			$LookupSQL = "select ";
			$LookupSQL .= GetFullFieldName("baid", $strTableName, true);
					if($linkFieldName != $dispfield)
				$LookupSQL .= ",namaBahagian";
		}
	}
