...
$layout->blocks["top"][] = "details";$page_layouts["jawatan_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "jawatan_inline_edit.htm" : "jawatan_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["jaid"] = urldecode(postvalue("editid1"));
$savedKeys["jaid"] = urldecode(postvalue("editid1"));
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["jaid"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: jawatan_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: jawatan_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("jaid", $data)));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='jawatan_view.php?".$viewlink."'\"");
