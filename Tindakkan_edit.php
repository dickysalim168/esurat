...
include("include/Tindakkan_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["Tindakkan_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "Tindakkan_inline_edit.htm" : "Tindakkan_edit.htm";
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: Tindakkan_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: Tindakkan_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Tindakkan";
		$options['firstTime'] = 1;
		
		$strTableName = $dpParams['strTableNames'][$d];
		
		if(!CheckSecurity(@$_SESSION["_".$strTableName."_OwnerID"],"Search")){
			$strTableName = "Tindakkan";
			continue;
		}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='Tindakkan_view.php?".$viewlink."'\"");
