...
$layout->blocks["top"][] = "details";$page_layouts["UserView_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "UserView_inline_add.htm";
else
	$templatefile = "UserView_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: UserView_add.php');
...
//	insert ownerid value if exists
	if($strOriginalTableName == $pageObject->pSet->getOwnerTable("pid"))
		$avalues["pid"] = prepare_for_db("pid",$_SESSION["_".$strTableName."_OwnerID"]);
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'UserView_edit.php?'.$keylink.'\'>'."Kemaskini".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'UserView_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: UserView_".$pageObject->getPageType().".php");
...
//	insert default ownerid value if exists
	if($strOriginalTableName == $pageObject->pSet->getOwnerTable("pid"))
		$defvalues["pid"] = prepare_for_db("pid",$_SESSION["_".$strTableName."_OwnerID"]);
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$showDetailKeys["pergerakan"]["masterkey1"] = $data["sid"];	
