...
require_once("include/pengguna_variables.php");
...
$layout = new TLayout("register_basic_left", "Purific1Aqua", "MobileAqua");
$layout->version = 2;
...
$layout = new TLayout("register_success2", "Purific1Aqua", "MobileAqua");
$layout->version = 2;
$layout->blocks["top"] = array();
...
//	processing NoKP
	$control_NoKP = $pageObject->getControl("NoKP", $id);
	$control_NoKP->readWebValue($values, $blobfields, NULL, NULL, $filename_values);
...
//	processing Email
	...
	$strUsername = $values["NoKP"];
	$strPassword = $values["Password"];

	if($regCipherer->isFieldEncrypted("NoKP"))
		$sUsername = $regCipherer->MakeDBValue("NoKP", $strUsername, "", true);	
	else 
		$sUsername = add_db_quotes("NoKP",$strUsername);
...
//	check if entered username already exists
	...
	{
		$strSQL = "select count(*) from ". $pageObject->connection->addTableWrappers( "pengguna" ) 
			. " where " . 
			$pageObject->getFieldSQLDecrypt("NoKP") . "=" . $sUsername;
	...
	if($retval)
	{
		$retval = DoInsertRecord("pengguna", $values, $blobfields, $pageObject);
	}
