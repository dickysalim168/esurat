...
$layout->blocks["top"][] = "details";$page_layouts["InboxPergerakanSurat_add"] = $layout;

$layout = new TLayout("tab","PurificAqua","MobileAqua");
...
$layout->blocks["bare"][] = "1";$page_layouts["InboxPergerakanSurat_add_PTG1"] = $layout;
...
$layout->skins["1"] = "fields";
$layout->blocks["bare"][] = "1";$page_layouts["InboxPergerakanSurat_add_Agensi1"] = $layout;
...
$layout->skins["1"] = "fields";
$layout->blocks["bare"][] = "1";$page_layouts["InboxPergerakanSurat_add_Awam1"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "InboxPergerakanSurat_inline_add.htm";
else
	$templatefile = "InboxPergerakanSurat_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: InboxPergerakanSurat_add.php');
...
//	insert ownerid value if exists
	if($strOriginalTableName == $pageObject->pSet->getOwnerTable("iid"))
		$avalues["iid"] = prepare_for_db("iid",$_SESSION["_".$strTableName."_OwnerID"]);
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'InboxPergerakanSurat_edit.php?'.$keylink.'\'>'."Kemaskini".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'InboxPergerakanSurat_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: InboxPergerakanSurat_".$pageObject->getPageType().".php");
...
//	insert default ownerid value if exists
	if($strOriginalTableName == $pageObject->pSet->getOwnerTable("iid"))
		$defvalues["iid"] = prepare_for_db("iid",$_SESSION["_".$strTableName."_OwnerID"]);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "InboxPergerakanSurat";
		$options['firstTime'] = 1;
