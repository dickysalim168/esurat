...
$layout->blocks["top"][] = "search";$page_layouts["suratview_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "suratview_search.htm";
...
$params['shortTableName'] = 'suratview';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("Minit"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("Minit")] = GetTableURL($pageObject->pSet->getLookupTable("Minit"));
	
	$pageObject->fillFieldToolTips("Minit");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("Minit");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("Minit_label","<label for=\"".GetInputElementId("Minit", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("Minit_label", true);
	
	$xt->assign("Minit_fieldblock", true);
	$xt->assignbyref("Minit_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("Minit_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("Minit_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_Minit", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("Minit");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Minit", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Minit", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
