...
$layout->blocks["top"][] = "details";$page_layouts["bahagian_edit"] = $layout;
...
$templatefile = "bahagian_edit.htm";
...
//	processing nama - end
//	processing penerangan - begin
	$condition = 1;

	if($condition)
	{
		$control_penerangan = $pageObject->getControl("penerangan", $id);
		$control_penerangan->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: bahagian_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: bahagian_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["penerangan"] = $evalues["penerangan"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("penerangan_label","<label for=\"".GetInputElementId("penerangan", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='bahagian_view.php?".$viewlink."'\"");
