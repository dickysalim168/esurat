...
$layout->blocks["top"][] = "search";$page_layouts["pejawatan_staffdetails_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "pejawatan_staffdetails_search.htm";
...
$params['shortTableName'] = 'pejawatan_staffdetails';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("staff_id"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("staff_id")] = GetTableURL($pageObject->pSet->getLookupTable("staff_id"));
	
	$pageObject->fillFieldToolTips("staff_id");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("staff_id");
	...
	{
		$firstFieldParams['fName'] = "staff_id";
	...
	if(isEnableSection508())
		$xt->assign_section("staff_id_label","<label for=\"".GetInputElementId("staff_id", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("staff_id_label", true);
	
	$xt->assign("staff_id_fieldblock", true);
	$xt->assignbyref("staff_id_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("staff_id_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("staff_id_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_staff_id", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("staff_id");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"staff_id", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"staff_id", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
