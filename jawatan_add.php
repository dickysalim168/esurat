...
$layout->blocks["top"][] = "details";$page_layouts["jawatan_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "jawatan_inline_add.htm";
else
	$templatefile = "jawatan_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: jawatan_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'jawatan_edit.php?'.$keylink.'\'>'."Kemaskini".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'jawatan_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: jawatan_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["jaid"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["jaid"]=postvalue("editid1");
	}
