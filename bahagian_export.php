...
$layout->blocks["top"][] = "export";$page_layouts["bahagian_export"] = $layout;
...
$xt->display("bahagian_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=bahagian.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=bahagian.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=bahagian.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["penerangan"] = $pageObject->showDBValue("penerangan", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=bahagian.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["penerangan"] = $pageObject->getViewControl("penerangan")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["penerangan"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Penerangan").'</td>';	
	}
	else
	{
		echo "<td>"."Nama"."</td>";
		echo "<td>"."Penerangan"."</td>";
	}
	echo "</tr>";
			$totals = array();
		$totals["nama"] = array("value" => 0, "numRows" => 0);
		$totalsFields[] = array('fName'=>"nama", 'totalsType'=>'', 'viewFormat'=>"");
			$totals["penerangan"] = array("value" => 0, "numRows" => 0);
		$totalsFields[] = array('fName'=>"penerangan", 'totalsType'=>'', 'viewFormat'=>"");
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["penerangan"]);
					else
						echo $values["penerangan"];
			echo '</td>';
			echo "</tr>";
		}
