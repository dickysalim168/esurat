...
add_nocache_headers();
include("include/pergerakanbahagian_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["pergerakanbahagian_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pergerakanbahagian_inline_add.htm";
else
	$templatefile = "pergerakanbahagian_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pergerakanbahagian_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing KepadaBahagian - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_KepadaBahagian = $pageObject->getControl("KepadaBahagian", $id);
		$control_KepadaBahagian->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing KepadaBahagian - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="pergerakan")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["pid"]==""){
			$avalues["pid"] = prepare_for_db("pid",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			
	}
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="PergerakanSurat")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
...
//	processing KepadaBahagian - start
			...
			if($inlineAddOption)
			{
				$control_KepadaBahagian->afterSuccessfulSave();
			}
//	processing KepadaBahagian - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pergerakanbahagian_edit.php?'.$keylink.'\'>'."Sunting".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pergerakanbahagian_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pergerakanbahagian_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["pbid"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["pbid"]=postvalue("editid1");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="pergerakan")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["pid"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($readavalues)
{
	$defvalues["KepadaBahagian"]=@$avalues["KepadaBahagian"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("KepadaBahagian"))
		$xt->assign("KepadaBahagian_fieldblock",true);
	else
		$xt->assign("KepadaBahagian_tabfieldblock",true);
	$xt->assign("KepadaBahagian_label",true);
	if(isEnableSection508())
		$xt->assign_section("KepadaBahagian_label","<label for=\"".GetInputElementId("KepadaBahagian", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["pbid"]));
...
//	KepadaBahagian
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("KepadaBahagian", $data, $keylink);
		$showValues["KepadaBahagian"] = $value;
		$showFields[] = "KepadaBahagian";
				$showRawValues["KepadaBahagian"] = substr($data["KepadaBahagian"],0,100);
	}	
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pergerakanbahagian";
		$options['firstTime'] = 1;
