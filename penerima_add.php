...
$layout->blocks["top"][] = "details";$page_layouts["penerima_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "penerima_inline_add.htm";
else
	$templatefile = "penerima_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: penerima_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'penerima_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'penerima_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: penerima_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["peid"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["peid"]=postvalue("editid1");
	}
