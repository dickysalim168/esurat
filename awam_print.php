...
$layout->blocks["top"][] = "pdf";$page_layouts["awam_print"] = $layout;
...
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "penerangan";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("penerangan");
$fieldsArr[] = $arr;
...
//	fill $rowinfo array
	...
	while($data && ($all || $recno<=$PageSize))
	{
		...
		for($col=1;$data && ($all || $recno<=$PageSize) && $col<=1;$col++)
		{
			...
			$records++;
			$keylink="";
			$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["awid"]));

//	nama - 
			$record["nama_value"] = $pageObject->showDBValue("nama", $data, $keylink);
			$record["nama_class"] = $pageObject->fieldAlignClass("nama");
//	penerangan - 
			$record["penerangan_value"] = $pageObject->showDBValue("penerangan", $data, $keylink);
			$record["penerangan_class"] = $pageObject->fieldAlignClass("penerangan");
			if($col<$colsonpage)
...
$xt->assign("penerangan_fieldheadercolumn",true);
$xt->assign("penerangan_fieldheader",true);
$xt->assign("penerangan_fieldcolumn",true);
$xt->assign("penerangan_fieldfootercolumn",true);
