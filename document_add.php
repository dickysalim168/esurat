...
add_nocache_headers();
include("include/document_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["document_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "document_inline_add.htm";
else
	$templatefile = "document_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: document_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing thumb - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_thumb = $pageObject->getControl("thumb", $id);
		$control_thumb->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing thumb - end
...
//	insert masterkey value if exists and if not specified
	...
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="UserView")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
...
//	processing thumb - start
			...
			if($inlineAddOption)
			{
				$control_thumb->afterSuccessfulSave();
			}
//	processing thumb - end
//	processing nama - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'document_edit.php?'.$keylink.'\'>'."Kemaskini".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'document_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: document_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["did"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["did"]=postvalue("editid1");
	}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("thumb"))
		$xt->assign("thumb_fieldblock",true);
	else
		$xt->assign("thumb_tabfieldblock",true);
	$xt->assign("thumb_label",true);
	if(isEnableSection508())
		$xt->assign_section("thumb_label","<label for=\"".GetInputElementId("thumb", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["did"]));
...
//	did
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("did", $data, $keylink);
		$showValues["did"] = $value;
		$showFields[] = "did";
				$showRawValues["did"] = substr($data["did"],0,100);
	}	
//	nama
	$display = false;
...
//	thumb
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("thumb", $data, $keylink);
		$showValues["thumb"] = $value;
		$showFields[] = "thumb";
				$showRawValues["thumb"] = substr($data["thumb"],0,100);
	}	
