...
$layout->blocks["top"][] = "details";$page_layouts["Copy_of_pejawatan_staffdetails_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "Copy_of_pejawatan_staffdetails_inline_edit.htm" : "Copy_of_pejawatan_staffdetails_edit.htm";
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: Copy_of_pejawatan_staffdetails_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: Copy_of_pejawatan_staffdetails_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Copy of pejawatan_staffdetails";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='Copy_of_pejawatan_staffdetails_view.php?".$viewlink."'\"");
