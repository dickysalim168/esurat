...
$layout->blocks["top"][] = "details";$page_layouts["jenis_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "jenis_inline_add.htm";
else
	$templatefile = "jenis_add.htm";
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'jenis_edit.php?'.$keylink.'\'>'."Kemaskini".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'jenis_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: jenis_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["jid"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["jid"]=postvalue("editid1");
	}
