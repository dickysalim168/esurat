...
$layout->blocks["top"][] = "details";$page_layouts["imbasan_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "imbasan_inline_add.htm";
else
	$templatefile = "imbasan_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: imbasan_add.php');
...
//	processing sid - end
//	processing Imbasan - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Imbasan = $pageObject->getControl("Imbasan", $id);
		$control_Imbasan->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'imbasan_edit.php?'.$keylink.'\'>'."Kemaskini".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'imbasan_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: imbasan_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["imid"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["imid"]=postvalue("editid1");
	}
...
if($readavalues)
{
	...
	$defvalues["Imbasan"]=@$avalues["Imbasan"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("Imbasan_label","<label for=\"".GetInputElementId("Imbasan", $id, PAGE_ADD)."\">","</label>");
