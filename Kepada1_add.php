...
$layout->blocks["top"][] = "details";$page_layouts["Kepada1_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "Kepada1_inline_add.htm";
else
	$templatefile = "Kepada1_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: Kepada1_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'Kepada1_edit.php?'.$keylink.'\'>'."Kemaskini".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'Kepada1_view.php?'.$keylink.'\'>'."Lihat".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: Kepada1_".$pageObject->getPageType().".php");
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="surat" && postvalue("field")=="individu") 
	{
		$lookupFieldName = "individu";
		$lookupTable = "Kepada1";
		$linkFieldName = "staff_id";
		$dispfield = "name";
		...
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "surat", "individu");
			...
			$LookupSQL = "select ";
			$LookupSQL .= GetFullFieldName("staff_id", $strTableName, true);
					if($linkFieldName != $dispfield)
				$LookupSQL .= ",name";
		}
	}
	if(postvalue("table")=="menuview" && postvalue("field")=="individu") 
	{
		$lookupFieldName = "individu";
		$lookupTable = "Kepada1";
		$linkFieldName = "staff_id";
		$dispfield = "name";
		// Table which contain lookup field!
		$lookupPSet = new ProjectSettings("menuview", PAGE_ADD);
		$nLookupType = $lookupPSet->getLookupType($lookupFieldName);
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "menuview", "individu");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
