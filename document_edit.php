...
$layout->blocks["top"][] = "details";$page_layouts["document_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "document_inline_edit.htm" : "document_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["did"] = urldecode(postvalue("editid1"));
$savedKeys["did"] = urldecode(postvalue("editid1"));
...
//	processing nama - end
//	processing thumb - begin
	$condition = 1;

	if($condition)
	{
		$control_thumb = $pageObject->getControl("thumb", $id);
		$control_thumb->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["did"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: document_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: document_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("thumb_label","<label for=\"".GetInputElementId("thumb", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("did", $data)));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='document_view.php?".$viewlink."'\"");
